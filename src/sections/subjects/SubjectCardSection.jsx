...
function SubjectCardSection({ subject, aosDelay = 0 }) {
  return (
    <article
      data-aos="fade-up"
    ...
    >
      ...
      <div className="mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-cyan-300/10 transition-colors duration-300 group-hover:bg-cyan-400/20">
        <BookOpenIcon className="h-7 w-7 text-cyan-300 transition-colors duration-300 group-hover:text-cyan-200" />
      </div>

      <h3 className="mb-2 text-xl font-bold leading-tight text-white">{subject.name}</h3>
      <p className="mb-1 text-sm font-semibold text-cyan-300 font-secondary">{subject.standard}</p>
      <p className="mb-4 text-sm text-slate-300 font-secondary">Teacher: {subject.teacher}</p>
      <p className="mb-5 text-sm text-slate-300 font-secondary">Weekly Periods: {subject.periodsPerWeek}</p>
...
export default SubjectCardSection
